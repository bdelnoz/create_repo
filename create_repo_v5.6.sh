...
# Version : v5.6 - Date : 2025-12-29
# Compatible avec : create_gitignore.sh v1.0
# Changelog v5.6 :
# - Intégration gestion robuste push pour tous visibilités (incl. public)
# - Push tente normal → si rejeté : fetch + force-with-lease sécurisé
# - Appliqué à main et working
# - Commit seulement si changements staging (évite commits vides)
# - Logs précis en cas récupération distante
# - Conservation 100% script précédent (règle d'or)
# - Log dans creation_log/ (auto-créé, auto-exclu .gitignore)
# - Timestamp unique log pour traçabilité
# Changelog v5.5 :
...
LOG_FILE="${LOG_DIR}/log.create_repo.v5.6.$(date +%Y%m%d_%H%M%S).log"
log() {
...
COMPORTEMENT v5.6:
  • Gestion robuste push (fetch + force-with-lease) pour public/private
...
╚════════════════════════════════════════════════════════════════════════════╝
v5.6 - 2025-12-29 : ROBUSTESSE PUSH PUBLIC
  • Intégration gestion push robuste pour --public/--private
  • Push normal → si rejeté : fetch + force-with-lease
  • Appliqué main/working
  • Commit si changements staging
  • Logs cas récupération distante
  • Conservation 100% script précédent
...
create_repo() {
    ...
    if [ "$DRY_RUN" = false ]; then
        ...
        if git status --porcelain | grep -q .; then
            git commit -m "Initial commit - full local files" || { log "✗ ERREUR git commit"; exit 1; }
        ...
        if git push -u origin main 2>/dev/null; then
            ...
            log "⚠ Push rejeté → récupération distante requise"
            git fetch origin || { log "✗ ERREUR git fetch"; exit 1; }
            git push -u origin main --force-with-lease || { log "✗ ERREUR push force-with-lease"; exit 1; }
            log "→ Push forcé avec lease réussi (dépôt distant récupéré)"
        fi
    fi
    ...
    if [ "$DRY_RUN" = false ]; then
        ...
        if git push -u origin working 2>/dev/null; then
            log "→ Push vers working réussi"
        else
            log "⚠ Push working rejeté → récupération distante requise"
            git fetch origin || { log "✗ ERREUR git fetch"; exit 1; }
            git push -u origin working --force-with-lease || { log "✗ ERREUR push working force-with-lease"; exit 1; }
            log "→ Push working forcé avec lease réussi"
        fi
    fi
