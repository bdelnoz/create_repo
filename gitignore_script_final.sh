...
while [[ $# -gt 0 ]]; do
    case "$1" in
        --list) list_templates ;;
        --simulate|-s) DRY_RUN=true; log "⚠ MODE SIMULATION ACTIVÉ"; shift ;;
        --no-log) NO_LOG=true; shift ;;
        --auto-append) AUTO_APPEND=true; shift ;;
        --help|-h) print_help ;;
        *) TEMPLATES+=("$1"); shift ;;
    esac
done

[ ${#TEMPLATES[@]} -eq 0 ] && { log "✗ ERREUR : Aucun template spécifié"; exit 1; }
[ ! -d "$TEMPLATES_DIR" ] && { log "✗ ERREUR : Dossier templates introuvable : $TEMPLATES_DIR"; exit 1; }
